...
import (
	...
	httpHandler "github.com/m-t-a97/go-better-auth/internal/delivery/http"
	"github.com/m-t-a97/go-better-auth/internal/domain"
	"github.com/m-t-a97/go-better-auth/internal/infrastructure/postgres"
...
type Handlers struct {
	// Authentication
	SignUpEmail http.HandlerFunc
	SignInEmail http.HandlerFunc
...
func New(config *Config) (*GoBetterAuth, error) {
	...
	// Create HTTP handler
	handler := httpHandler.NewHandler(authUseCase, oauthUseCase)
...
// GetHandlers returns individual HTTP handler functions that can be registered with any framework
func (ba *GoBetterAuth) GetHandlers() *Handlers {
	return &Handlers{
		// Authentication handlers
		SignUpEmail: ba.handler.SignUpEmail,
		SignInEmail: ba.handler.SignInEmail,
		SignOut:     ba.handler.SignOut,
		GetSession:  ba.handler.GetSession,
		// Email verification handlers
		SendVerificationEmail: ba.handler.SendVerificationEmail,
		VerifyEmail:           ba.handler.VerifyEmail,
		// Password management handlers
		RequestPasswordReset: ba.handler.RequestPasswordReset,
		ResetPassword:        ba.handler.ResetPassword,
		ChangePassword:       ba.handler.ChangePassword,
		// OAuth handlers
		OAuthAuthorize: ba.handler.OAuthAuthorize,
		OAuthCallback:  ba.handler.OAuthCallback,
	}
}

// GetMiddleware returns middleware functions that can be used with any framework
func (ba *GoBetterAuth) GetMiddleware() *Middleware {
	return &Middleware{
		SessionAuth: ba.handler.SessionAuthMiddleware,
		CORS:        ba.handler.CORSMiddleware,
	}
}
...
// Handler returns the HTTP handler (deprecated: use GetHandlers() instead)
// Kept for backward compatibility
func (ba *GoBetterAuth) Handler() http.Handler {
	return ba.handler.SetupRouter()
}
